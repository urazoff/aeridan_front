...
class KatexTool {
  /**
   * Allow to press Enter inside the KatexTool textarea
   * @returns {boolean}
   * @public
   */
  ...
  /**
   * @typedef {Object} KatexData — plugin saved data
   * @param {String} katex - previously saved plugin katex
   */

  /**
   * Render plugin`s main Element and fill it with saved data
   *
   * @param {KatexData} data — previously saved plugin katex
  ...
  constructor({data, config, api}) {
    ...
    this.placeholder = config.placeholder || KatexTool.DEFAULT_PLACEHOLDER;

    this.CSS = {
      baseClass: this.api.styles.block,
      input: this.api.styles.input,
      wrapper: 'ce-katex',
      textarea: 'ce-katex__textarea',
      preview: 'ce-katex__preview'
    };
  ...
  drawView() {
    ...
    textarea.textContent = this.data.katex;
  ...
  /**
   * Return Tool's view
   * @returns {HTMLDivElement} this.nodes.holder - Katex's wrapper
   * @public
   */
  render() {
   ...
   * @param {HTMLDivElement} KatexWrapper - KatexTool's wrapper, containing textarea with Katex
   * @returns {KatexData} - saved plugin Katex
   * @public
   */
  save(KatexWrapper) {
    return {
      katex: KatexWrapper.querySelector('textarea').value
    };
  }
   ...
   * @param {KatexData} data
   */
  set data(data) {
    this._data = data;

    if (this.nodes.textarea) {
      this.nodes.textarea.textContent = data.katex;
    }
  }

  validate(savedData) {
    if (!savedData.katex.trim()) {
      return false;
    }
    return true;
  }
  ...
  /**
   * Default placeholder for KatexTool's textarea
   *
   * @public
   * @returns {string}
   */
   ...
   *  Provides configuration to handle Katex tag.
   *
   * @static
   * @return {{tags: string[]}}
   */
